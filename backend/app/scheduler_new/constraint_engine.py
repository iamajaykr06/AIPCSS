...
class ConstraintEngine:
    ...
    def __init__(self, problem: SchedulingProblem, debug: bool = False):
        ...
        self.faculty_schedule: Dict[int, Dict[Tuple[str, str], Set[Tuple[int, int]]]] = defaultdict(
            lambda: defaultdict(set)
        )
        self.room_schedule: Dict[int, Dict[Tuple[str, str], Set[Tuple[int, int]]]] = defaultdict(
            lambda: defaultdict(set)
        )
        self.section_schedule: Dict[int, Dict[Tuple[str, str], Set[Tuple[int, int]]]] = defaultdict(
            lambda: defaultdict(set)
        )
    ...
    def is_consistent(self, entry: ScheduleEntry) -> Tuple[bool, Optional[str]]:
        ...
        if sc_key in self.section_course_faculty:
            ...
            if entry.faculty_id != assigned_faculty_id:
                ...
                return (
                    False,
                    f"Course {course.code} for section {section.name} must be "
                    f"taught by {assigned_name} (consistency constraint)",
                )
        ...
        if assigned_faculty_id is not None:
            ...
            if assigned_faculty:
                ...
                if not self.faculty_schedule[assigned_faculty_id][slot_key]:
                    # Check max hours
                    if (
                        self.faculty_hours[assigned_faculty_id] < assigned_faculty.max_hours_per_week
                        and self.faculty_daily_hours[assigned_faculty_id][timeslot.day]
                        < assigned_faculty.max_hours_per_day
                    ):
    ...
    def count_violations(self) -> Dict[str, int]:
        ...
        violations = {"faculty_overlap": 0, "room_overlap": 0, "section_overlap": 0, "faculty_overload": 0}
