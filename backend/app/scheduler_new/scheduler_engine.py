...
class SchedulerEngine:
    ...
    def __init__(
        self, problem: SchedulingProblem, debug: bool = False, max_retries: int = 3, time_limit_seconds: float = 60.0
    ):
            ...
            for course_id in section.course_ids:
                ...
                if course.is_lab():
                    ...
                    for _ in range(course.get_hours_needed()):
                        instances.append(AssignmentVariable(section_id=section.id, course_id=course_id, hours_needed=1))
        return instances
    ...
    def _compute_static_domain(self, var: AssignmentVariable) -> List[DomainValue]:
        ...
        for timeslot in self.problem.timeslots:
            ...
            valid_faculty = self.constraint_engine.get_valid_faculty(var.section_id, var.course_id, anchor_slot)

            # Get valid rooms for this (section, course, timeslot)
            valid_rooms = self.constraint_engine.get_valid_rooms(var.section_id, var.course_id, anchor_slot)

            # Create all combinations
            for faculty_id in valid_faculty:
                for room_id in valid_rooms:
                    domain_values.append(DomainValue(faculty_id=faculty_id, room_id=room_id, timeslot=anchor_slot))

        return domain_values

    def _compute_domain(self, var: AssignmentVariable, assigned_slots: Set[Timeslot]) -> List[DomainValue]:
        """
    ...
    def _find_consecutive_slots(
        self, start_slot: Timeslot, hours_needed: int, assigned_slots: Set[Timeslot]
    ) -> Optional[List[Timeslot]]:
        """Find consecutive timeslots for lab sessions"""
        # Group slots by day
        day_slots = [t for t in self.problem.timeslots if t.day == start_slot.day and t not in assigned_slots]
    ...
    def _mrv_heuristic(
        self, variables: List[AssignmentVariable], assigned_slots: Set[Timeslot]
    ...
    def _lcv_heuristic(self, var: AssignmentVariable, domain: List[DomainValue]) -> List[DomainValue]:
    ...
    def _forward_check(
        ...
        affected_vars = [v for v in remaining_vars if not v.assigned and self._shares_resource(v, entry)]
    ...
    def _backtrack(
        ...
        for value in domain:
            ...
            if self.debug:
                self.logger.debug(
                    f"Assigned depth {depth}: section={var.section_id}, "
    ...
    def solve(self, progress_callback: Optional[Callable[[int, str], None]] = None) -> ScheduleResult:
        ...
        if total_classes == 0:
            return ScheduleResult(success=False, error_message="No classes to schedule")
