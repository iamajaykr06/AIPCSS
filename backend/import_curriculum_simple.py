...
def import_curriculum_from_excel(excel_file_path):
    ...
    with app.app_context():
        try:
            ...
            print("💡 Make sure the Excel file has columns: code, Name, Type, Semester, Program, Department")
...
def update_batch_current_semesters():
    ...
    with app.app_context():
        # Update BCA CS batches
        bca_cs_2023 = Batch.query.filter_by(name='Batch 2023').filter_by(program_id=2).first()
        bca_cs_2024 = Batch.query.filter_by(name='Batch 2024').filter_by(program_id=2).first()
        
        if bca_cs_2023:
            bca_cs_2023.current_semester = 6
            print(f"📚 Updated {bca_cs_2023.name} to Semester {bca_cs_2023.current_semester}")
        
        if bca_cs_2024:
            bca_cs_2024.current_semester = 4
            print(f"📚 Updated {bca_cs_2024.name} to Semester {bca_cs_2024.current_semester}")
...
if __name__ == '__main__':
    ...
    excel_path = "courses.xlsx"  # Put your Excel file in the same directory
    
    print("🔧 Starting curriculum import...")
