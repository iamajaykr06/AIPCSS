...
class GreedySchedulerEngine:
    ...
    def solve(self, progress_callback: Optional[Callable[[int, str], None]] = None) -> ScheduleResult:
        ...
        for i, (difficulty, c_idx, sec_id, crs_id, hrs) in enumerate(class_difficulties):
            if progress_callback and i % 50 == 0:
                progress_callback(30 + (60 * i // len(class_difficulties)), f"Scheduled {i}/{len(classes)} classes...")
            ...
            possible_r = [
                ...
                if r.can_accommodate(section.student_count) and r.can_be_used_by_program(None, section_dept_id)
            ]
    ...
    def _try_assign(
        self, c_idx, sec_id, crs_id, hrs, course, section, possible_f, possible_r, allowed_starts, timeslot_list
    ) -> bool:
