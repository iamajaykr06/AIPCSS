...
class GeneticSchedulerEngine:
            ...
            for course_id in section.course_ids:
                ...
                if course:
                    if course.course_type == "Lab":
                        instances.append(
                            AssignmentVariable(
                                section_id=section.id, course_id=course_id, hours_needed=course.get_hours_needed()
                            )
                        )
                    else:
                        for _ in range(course.get_hours_needed()):
                            instances.append(
                                AssignmentVariable(section_id=section.id, course_id=course_id, hours_needed=1)
                            )
        return instances
    ...
    def _precompute_domains(self):
        ...
        for var in self.variables:
            ...
            for timeslot in self.problem.timeslots:
                ...
                valid_options.append({"timeslot": timeslot, "faculties": valid_faculties, "rooms": valid_rooms})
        ...
        self.variables.sort(
            key=lambda var: sum(
                len(opt["faculties"]) * len(opt["rooms"])
                for opt in self.domains.get((var.section_id, var.course_id, var.hours_needed), [])
            )
        )
    ...
    def _evaluate(self, individual: List[Optional[DomainValue]]) -> Tuple[int, Dict[str, int]]:
        ...
        for i, var in enumerate(self.variables):
            ...
            # Repetition Constraints: Ensure 1-hour theory constraints don't
            # repeat uniformly on the exact same day for the batch
    ...
    def _enforce_consistency_in_individual(
        self, individual: List[Optional[DomainValue]]
    ) -> List[Optional[DomainValue]]:
        """
    ...
    def _local_search_hill_climbing(
        self, individual: List[Optional[DomainValue]], max_iterations: int = 10
    ) -> List[Optional[DomainValue]]:
        """
    ...
    def _repair_individual(self, individual: List[Optional[DomainValue]]) -> List[Optional[DomainValue]]:
        ...
        for i, var in enumerate(self.variables):
            ...
            for slot_key in slot_keys:
                if (
                    (val.faculty_id, slot_key) in faculty_schedule
                    or (val.room_id, slot_key) in room_schedule
                    or (var.section_id, slot_key) in section_schedule
                ):
                    has_conflict = True
                    break
        ...
        conflict_indices.sort(
            key=lambda i: len(
                self.domains.get(
                    (self.variables[i].section_id, self.variables[i].course_id, self.variables[i].hours_needed), []
                )
            )
        )
    ...
    def solve(self, progress_callback: Optional[Callable[[int, str], None]] = None) -> ScheduleResult:
        ...
        for var in self.variables:
            ...
            if not self.domains.get(key):
                ...
                return ScheduleResult(
                    success=False,
                    error_message=f"No valid assignment possible for section {var.section_id} course {var.course_id}.",
                )
        ...
        for generation in range(adaptive_generations):
                ...
                if conflicts < best_conflicts:
                    ...
                    if best_conflicts == 0:
                        if self.use_local_search:
                            best_individual = self._local_search_hill_climbing(
                                best_individual, self.local_search_intensity
                            )
                        break
            ...
            if progress_callback and generation % 20 == 0:
                ...
                progress_callback(
                    progress_val, f"GA Gen {generation}, Best: {best_conflicts}, Mut: {self.current_mutation_rate:.3f}"
                )
