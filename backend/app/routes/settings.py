...
def update_schedule_settings():
    ...
    if "time_slots" in data:
        ...
        if not isinstance(time_slots, list):
            ...
            for i, slot in enumerate(time_slots):
                ...
                if not re.match(r"^\d{2}:\d{2}$", str(slot["start"])) or not re.match(
                    r"^\d{2}:\d{2}$", str(slot["end"])
                ):
        ...
        if not isinstance(breaks, list):
            ...
            for i, brk in enumerate(breaks):
                ...
                if "start" not in brk or "end" not in brk:
                    ...
                    continue
                if not re.match(r"^\d{2}:\d{2}$", str(brk["start"])) or not re.match(r"^\d{2}:\d{2}$", str(brk["end"])):
        ...
        for ts in settings.time_slots:
            ...
            for brk in settings.breaks:
                ...
                if not b_start or not b_end:
                    continue
                if (
                    (ts_start >= b_start and ts_start < b_end)
                    or (ts_end > b_start and ts_end <= b_end)
                    or (ts_start <= b_start and ts_end >= b_end)
                ):
                    overlap = True
                    break
    ...
    db.session.commit()
    return jsonify({"message": "Schedule settings updated", "settings": settings.to_dict()}), 200
...
def reset_schedule_settings():
    ...
    settings = ScheduleSettings.get_or_create_default()
    return jsonify({"message": "Schedule settings reset to defaults", "settings": settings.to_dict()}), 200
...
def preview_schedule():
    ...
    if not time_slots:
        ...
        while current < end_time:
            ...
            for b in sorted_breaks:
                ...
                if (
                    (current >= b_start and current < b_end)
                    or (slot_end > b_start and slot_end <= b_end)
                    or (current <= b_start and slot_end >= b_end)
                ):
                    overlap_break = b
                    break
            ...
            slots.append({"start": current, "end": slot_end, "label": f"Period {slot_num}"})
            current = slot_end
            slot_num += 1
        time_slots = slots
    ...
    preview = {
        ...
        "schedule_grid": {
            day: [
                {"time": slot, "is_break": any(slot["start"] == b["start"] and slot["end"] == b["end"] for b in breaks)}
